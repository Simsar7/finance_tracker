...
import { FiSave, FiDollarSign, FiCalendar, FiFileText, FiMinusCircle } from "react-icons/fi";
...
const SpendSavings = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Error spending saving:", err.response?.data || err.message);
      setError(
        err.response?.data?.detail
          ? JSON.stringify(err.response.data.detail, null, 2)
          : "Failed to spend savings. Please try again."
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
          <div className="p-6 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center">
              <FiMinusCircle className="mr-2" /> Spend from Savings
            </h2>
            <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
              Record money you've spent from your savings
            </p>
          </div>
          ...
          <div className="p-6">
            ...
            {success && (
              <div className="mb-4 p-3 bg-green-50 dark:bg-green-900/20 border-l-4 border-green-500 text-green-700 dark:text-green-300 rounded">
                <p>Amount successfully spent from savings!</p>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    <FiFileText className="inline mr-1" /> Reason (Optional)
                  </label>
                  <input
                    type="text"
                    name="reason"
              ...
              <div className="pt-4">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-70 transition duration-200 transform hover:scale-105"
                >
                  {loading ? (
                    <>
